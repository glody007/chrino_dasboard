...
export default {
  props: ['active', 'account', 'accountId', 'currencyCode'],
  components: {
    AlertMessage
  },
  data () {
  ...
  methods : {
    ...
    notFiat: function() {
      return this.currencyCode !== 'USD' && this.currencyCode !== 'USDT'
    },
    executeTransfer () {
...
<template>
    ...
    <div class="relative top-40 md:w-1/2 mx-auto shadow-lg rounded-md bg-white">
        ...
        <div class="flex justify-between bg-indigo-600 items-center text-white text-xl rounded-t-md px-4 py-2">
            <h3>{{ account }}: Dépot {{ currencyCode }}</h3>
            ...
            <div class="justify-center md:justify-start my-auto py-3 md:pt-0 px-2 md:px-8 lg:px-16">
              <form class="flex flex-col pt-1 md:pt-4 pb-3 md:pb-8" onsubmit="event.preventDefault();">
                  <div class="flex flex-col pt-4">
                      <label for="quantity" class="">Quantité {{ currencyCode }} à déposer:</label>
                      ...
                      <label v-if="notFiat()" for="prix" class="">Prix d'achat:</label>
                      <input v-if="notFiat()" v-model="taux"  type="number" id="prix" placeholder="0" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mt-1 leading-tight focus:outline-none focus:shadow-outline" />
                  </div>
              </form>
            </div>
            <!-- End Transfer -->
        ...
        <div class="px-4 py-2 border-t border-t-gray-500 flex justify-end items-center space-x-4">
          <button
            ...
            class="inline-flex gap-x-2 items-center bg-indigo-600 py-2.5 px-6 text-white rounded-xl hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
          >
