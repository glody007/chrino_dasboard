<template>
  <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
      <div class="container mx-auto px-6 py-8">
          <h3 class="text-gray-700 text-3xl font-medium">Utilisateurs</h3>

          <DialogUserDetails v-if="active" :user="user" :active="active" @toggle="toggle()" />
                          ...
                          <tbody class="bg-white">
                              <tr v-for="user in users" :key="user.username">
                                  ...
                                  <td
                                      ...
                                      <button @click="select(user)"  class="bg-blue-500 p-1 mr-1 rounded-xl text-white shadow-xl hover:shadow-inner focus:outline-none transition duration-500 ease-in-out  transform hover:-translate-x hover:scale-105">
                                          More
                                      </button>
                                  </td>
                              </tr>
...
import DialogUserDetails from  '../components/DialogUserDetails.vue'

export default {
  components: {
    AlertMessage, DialogUserDetails
  },
  data () {
    return {
      dropdownOpen: false,
  ...
  created () {
    this.fetchUsers()
    this.$store.commit('selectItem', 'users')
  },
  methods: {
    toggle: function(){
    ...
    getNumber(user) {
      if(user.profile) return user.profile.numero
      return ''
    }
  }
}
